            ...
            <div class="section flex justify-center items-center bg-white shadow-md">
                <div class="w-[600px] border-[3px] border-blue-400 pt-20 pb-20">
                    <h1 class="text-2xl font-bold text-center mb-6">
                        {{ (this.$route.query.id == null) ? 'CREATE' : 'UPDATE' }} PRODUCT
                    </h1>
                    <div class="register flex flex-col items-center">
                        ...
                        <form @submit.prevent="submitForm" class="w-full max-w-md">
                            ...
                            <textarea v-model="form.description" type="" placeholder="Enter Description" required
                                class="block w-full p-2 mb-4 border border-gray-300 rounded" />
                            <input v-model="form.amount" type="number" placeholder="Enter Amount" required
                                class="block w-full p-2 mb-4 border border-gray-300 rounded" />
...
export default {
    ...
    methods: {
        async submitForm() {
            ...
            try {
                const response = await axios.post(`${this.apiUrl}auth/product`, formData, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    }
                });
                this.$showToast('PASS', response.data.message);
                clearCacheSpecific(`${this.apiUrl}auth/product?find=${''}&skip=${0}&limit=${10}`);
                setTimeout(() => this.$router.push({ name: 'Product' }), 1500);
            } catch (error) {
                this.$showToast("FAIL", error);
            } finally {
                this.loading = false;
